...
// the message object changes once, swapping in <StaticContent>. The wrapper stays
...
export const MessageBubble = memo(function MessageBubble({ message }: { message: UiMessage }) {
  ...
  return (
    <div
      className={`animate-msg-in max-w-[85%] rounded-2xl border px-3 py-2 text-sm ${tone}`}
      style={{ contain: 'content' }}
    >
      {message.streaming ? <LiveContent /> : <StaticContent content={message.content} />}
    </div>
  )
})
